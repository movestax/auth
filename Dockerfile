...
WORKDIR /go/src/github.com/movestax/auth
...
# Building stuff
COPY . /go/src/github.com/movestax/auth
...
FROM alpine:3
RUN adduser -D -u 1000 movestax

RUN apk add --no-cache ca-certificates
COPY --from=build /go/src/github.com/movestax/auth/auth /usr/local/bin/auth
COPY --from=build /go/src/github.com/movestax/auth/migrations /usr/local/etc/auth/migrations/
